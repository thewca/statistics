...
public class RecordEvolutionRepositoryJdbcImpl implements RecordEvolutionRepositoryJdbc {
    ...
    public void upsert(RegionDTO region, String eventId, LocalDate today) {
        MapSqlParameterSource paramsList = new MapSqlParameterSource().addValue(
                RecordEvolution.Fields.EVENT_ID.name(), eventId)
            .addValue(RecordEvolution.Fields.EVOLUTION.name(),
                jsonUtil.convertToJson(workData(region, today)));

        namedJdbcTemplate.update(
            LoadResourceUtil.getResource("db/query/recordevolution/upsert.sql"), paramsList);
    }

    @Override
    public int removeAll() {
        return namedJdbcTemplate.update(
            LoadResourceUtil.getResource("db/query/recordevolution/removeAll.sql"),
            new HashMap<>());
    ...
    public List<EventDto> getAvailableEvents() {
        return namedJdbcTemplate
            .query(
                LoadResourceUtil.getResource("db/query/recordevolution/getAvailableEvents.sql"),
